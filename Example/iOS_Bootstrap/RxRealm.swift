...
public extension Reactive where Base == Realm {
     ...
     */
    func add<S: Sequence>(update: Bool = false, onError: ((S?, Error)->Void)? = nil)
        ...
        return RealmObserver(realm: base) { realm, elements, error in
            ...
            do {
                try realm.write {
                    realm.add(elements, update: .modified)
                }
            } catch let e {
                onError?(elements, e)
            }
     ...
     */
    func add<O: Object>(update: Bool = false,
