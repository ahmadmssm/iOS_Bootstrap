...
public extension PrimitiveSequence where TraitType == SingleTrait, ElementType == Response {
    ...
    func processErrors(errorHandler: LucidErrorMessageProvider? = Singleton.sharedInstance.errorHandler) -> Single<ElementType> {
...
public extension PrimitiveSequence where TraitType == SingleTrait, ElementType == Response {
    ...
    func filter(statusCodes: [ClosedRange<Int>]) -> Single<ElementType> {
    ...
    func filterSuccessfulStatusCodes(code: Int? = nil, statusCodes: [ClosedRange<Int>] = []) -> Single<ElementType> {
        return flatMap { response -> Single<ElementType> in
    ...
    func filterSuccessfulStatusAndRedirectCodes(code: Int? = nil, statusCodes: [ClosedRange<Int>] = []) -> Single<ElementType> {
        return flatMap { response -> Single<ElementType> in
...
public extension PrimitiveSequence where TraitType == SingleTrait, ElementType == Response {
    ...
    func filterAndProcessErrors(statusCodes: [ClosedRange<Int>], errorHandler: LucidErrorMessageProvider? = Singleton.sharedInstance.errorHandler) -> Single<ElementType> {
        return flatMap { response -> Single<ElementType> in
    ...
    /// Filters out responses that do not have a status code >=200, <300 and don't fall within the given range, a MoyaError status code error errors when others are encountered.
    /// This function will also run any errors thrown through `.getLucidError()` for you to process error using Lucid.
    func filterSuccessfulStatusCodesAndProcessErrors(code: Int? = nil, statusCodes: [ClosedRange<Int>] = [], errorHandler: LucidErrorMessageProvider? = Singleton.sharedInstance.errorHandler) -> Single<ElementType> {
        return flatMap { response -> Single<ElementType> in
    ...
    /// Filters out responses that do not have a status code >=200, <400 and don't fall within the given range, a MoyaError status code error errors when others are encountered.
    /// This function will also run any errors thrown through `.getLucidError()` for you to process error using Lucid.
    func filterSuccessfulStatusAndRedirectCodesAndProcessErrors(code: Int? = nil, statusCodes: [ClosedRange<Int>] = [], errorHandler: LucidErrorMessageProvider? = Singleton.sharedInstance.errorHandler) -> Single<ElementType> {
        return flatMap { response -> Single<ElementType> in
