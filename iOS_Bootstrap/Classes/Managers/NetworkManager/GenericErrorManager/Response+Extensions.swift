...
public extension Response {
    ...
    func filterSuccessfulStatusCodes(code: Int? = nil, statusCodes: [ClosedRange<Int>] = []) throws -> Response {
    ...
    func filterSuccessfulStatusAndRedirectCodes(code: Int? = nil, statusCodes: [ClosedRange<Int>] = []) throws -> Response {
        var allStatusCodes = statusCodes
...
public extension Response {
    ...
    func filterAndProcessErrors(statusCodes: [ClosedRange<Int>], errorHandler: LucidErrorMessageProvider? = Singleton.sharedInstance.errorHandler) throws -> Response {
        var successfulStatusCode = false
        statusCodes.forEach { (statsCodesSet: ClosedRange<Int>) in
            if statsCodesSet.contains(statusCode) {
                successfulStatusCode = true
    ...
    /// Filters out responses that do not have a status code >=200, <300 and don't fall within the given range, generating a MoyaError status code error when others are encountered.
    /// This function will also run any errors thrown through `.getLucidError()` for you to process error using Lucid.
    func filterSuccessfulStatusCodesAndProcessErrors(code: Int? = nil, statusCodes: [ClosedRange<Int>] = [], errorHandler: LucidErrorMessageProvider? = Singleton.sharedInstance.errorHandler) throws -> Response {
        var allStatusCodes = statusCodes
    ...
    /// Filters out responses that do not have a status code >=200, <400 and don't fall within the given range, generating a MoyaError status code error when others are encountered.
    /// This function will also run any errors thrown through `.getLucidError()` for you to process error using Lucid.
    func filterSuccessfulStatusAndRedirectCodesAndProcessErrors(code: Int? = nil, statusCodes: [ClosedRange<Int>] = [], errorHandler: LucidErrorMessageProvider? = Singleton.sharedInstance.errorHandler) throws -> Response {
        var allStatusCodes = statusCodes
