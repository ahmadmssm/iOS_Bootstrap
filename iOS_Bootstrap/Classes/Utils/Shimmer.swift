...
//
// Ref : https://github.com/samhann/Loader.swift
...
open class Shimmer {
    ...
    public static func removeFromViews(_ views: [UIView]) {
        CATransaction.begin()
        views.forEach { $0.ld_removeLoader() }
        CATransaction.commit()
    }
    
    public static func addTo(_ list : ListLoadable) {
        self.addToViews(list.ld_visibleContentViews())
    }
    
    public static func removeFrom(_ list : ListLoadable) {
        self.removeFromViews(list.ld_visibleContentViews())
    }
}
